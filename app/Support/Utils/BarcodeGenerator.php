<?php

namespace App\Support\Utils;

class BarcodeGenerator
{
    /**
     * Generate HTML representation of Code128 barcode.
     */
    public static function generateHtml(string $code): string
    {
        $bars = self::code128ToBars($code);
        $html = '<div style="display: inline-block; white-space: nowrap; height: 36px; padding: 0; margin: 0 auto;">';

        foreach ($bars as $bar) {
            $color = $bar['color'] === 'B' ? '#000000' : '#ffffff';
            $width = $bar['width'] * 0.75;
            $html .= sprintf(
                '<div style="display: inline-block; width: %.1fpx; height: 100%%; background-color: %s;"></div>',
                $width,
                $color
            );
        }

        $html .= '</div>';

        return $html;
    }

    /**
     * Convert code string to barcode bar widths array.
     */
    private static function code128ToBars(string $code): array
    {
        $code128Patterns = [
            0 => '212222', 1 => '222122', 2 => '222221', 3 => '121223', 4 => '121322',
            5 => '131222', 6 => '122213', 7 => '122312', 8 => '132212', 9 => '221213',
            10 => '221312', 11 => '231212', 12 => '112232', 13 => '122132', 14 => '122231',
            15 => '113222', 16 => '123122', 17 => '123221', 18 => '223211', 19 => '221132',
            20 => '221231', 21 => '213212', 22 => '223112', 23 => '312131', 24 => '311222',
            25 => '321122', 26 => '321221', 27 => '312212', 28 => '322112', 29 => '322211',
            30 => '212123', 31 => '212321', 32 => '232121', 33 => '111323', 34 => '131123',
            35 => '131321', 36 => '112313', 37 => '132113', 38 => '132311', 39 => '211313',
            40 => '231113', 41 => '231311', 42 => '112133', 43 => '112331', 44 => '132131',
            45 => '113123', 46 => '113321', 47 => '133121', 48 => '313121', 49 => '211331',
            50 => '231131', 51 => '213113', 52 => '213311', 53 => '213131', 54 => '311123',
            55 => '311321', 56 => '331121', 57 => '312113', 58 => '312311', 59 => '332111',
            60 => '314111', 61 => '221411', 62 => '431111', 63 => '111224', 64 => '111422',
            65 => '121124', 66 => '121421', 67 => '141122', 68 => '141221', 69 => '112214',
            70 => '112412', 71 => '122114', 72 => '122411', 73 => '142112', 74 => '142211',
            75 => '241211', 76 => '221114', 77 => '413111', 78 => '241112', 79 => '134111',
            80 => '111242', 81 => '121142', 82 => '121241', 83 => '114212', 84 => '124112',
            85 => '124211', 86 => '411212', 87 => '421112', 88 => '421211', 89 => '212141',
            90 => '214121', 91 => '412121', 92 => '201141', 93 => '211411', 94 => '231141',
            95 => '114111', 96 => '114211', 97 => '411112', 98 => '411211', 99 => '211141',
            100 => '111412', 101 => '121141', 102 => '114121', 103 => '211412', 104 => '211214',
            105 => '211232', 106 => '2331112',
        ];

        $startCodeB = 104;
        $checksum = $startCodeB;
        $patternSequence = [$code128Patterns[$startCodeB]];

        $chars = str_split($code);
        foreach ($chars as $i => $char) {
            $ascii = ord($char);
            $val = $ascii - 32;
            if ($val < 0 || $val > 95) {
                $val = 0;
            }
            $checksum += $val * ($i + 1);
            $patternSequence[] = $code128Patterns[$val];
        }

        $checksumValue = $checksum % 103;
        $patternSequence[] = $code128Patterns[$checksumValue];
        $patternSequence[] = $code128Patterns[106]; // Stop pattern

        $fullPattern = implode('', $patternSequence);
        $bars = [];
        $isBlack = true;

        for ($k = 0; $k < strlen($fullPattern); $k++) {
            $width = (int) $fullPattern[$k];
            $bars[] = [
                'color' => $isBlack ? 'B' : 'W',
                'width' => $width,
            ];
            $isBlack = ! $isBlack;
        }

        return $bars;
    }
}
