...
class ProductService implements ProductServiceInterface
{
            ...
            foreach ($raws as $raw) {
                foreach ($raw as $row) {
                    ...
                    $newProduct = [
                        ...
                        'desc' => $row['deskripsi'] ?? Constants::EMPTY_STRING_VALUE,
                        'created_at' => $now,
                        'updated_at' => $now,
                    ];
    ...
    public function exportExcel(): BinaryFileResponse
    {
        try {
            ...
            $sheet->fromArray([
                ['Nama', 'Kategori', 'Satuan', 'Barcode', 'Stok', 'Harga Modal', 'Harga Jual', 'Status', 'Tipe Stok', 'Deskripsi', 'SKU'],
            ], null, 'A1');
            ...
            foreach ($products as $product) {
                $rows[] = [
                    ...
                    $product->cost_price,
                    $product->price,
                    $product->is_active ? Constants::ACTIVE_PRODUCT_TEMPLATE_VALUE : Constants::INACTIVE_PRODUCT_TEMPLATE_VALUE,
                    $product->is_unlimited ? Constants::UNLIMITED_TYPE_PRODUCT_TEMPLATE_VALUE : Constants::LIMITED_TYPE_PRODUCT_TEMPLATE_VALUE,
                    $product->desc,
                    $product->sku,
                ];
            }
