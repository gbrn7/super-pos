...
import { purchaseProduct as apiPurchaseProduct } from '@/routes/apiCapitalWallet';
...
export function PurchaseProductDialog({ onSuccess }: PurchaseProductDialogProps) {
    ...
    const schema = z.object({
        amount: z.number().min(0.01, t('validation.capital_wallet.min_amount', 'Nominal pembelian minimal Rp 0.01')),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const res = await axiosInstance.post(apiPurchaseProduct().url, formData);
                    ...
                    <FieldGroup>
                        <Field>
                            <label htmlFor="amount" className="text-sm font-medium text-foreground">
                                {t('page.capital_wallet.dialog_modal.purchase_product.amount_label', 'Nominal Pembelian')}
                             ...
                             <NumericFormat
                                ...
                                customInput={Input}
                                id="amount"
                                placeholder={t('page.capital_wallet.dialog_modal.purchase_product.amount_placeholder', 'Masukkan nominal pembelian produk')}
                        ...
                        <Field>
                            <label htmlFor="notes" className="text-sm font-medium text-foreground">
                                {t('page.capital_wallet.dialog_modal.purchase_product.notes_label', 'Catatan')}
                            </label>
                            <Textarea
                                id="notes"
                                name="notes"
                    ...
                    <DialogFooter>
                        <DialogClose asChild>
                            <Button
                                ...
                                disabled={loading}
                            >
                                {t('page.capital_wallet.dialog_modal.purchase_product.cancel_btn', 'Batal')}
                            </Button>
                        </DialogClose>
                        <Button type="submit" disabled={loading} className="bg-sky-600 hover:bg-sky-700 text-white">
                            {loading ? <Spinner /> : t('page.capital_wallet.dialog_modal.purchase_product.confirm_btn', 'Bayar')}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
