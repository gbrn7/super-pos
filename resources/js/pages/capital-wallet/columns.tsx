...
export const columns = ({ onInvoiceClick, onSortChange, orderBy, order }: ColumnProps): ColumnDef<CapitalWalletTransaction>[] => [
    {
        accessorKey: 'created_at',
        header: () => i18next.t('page.capital_wallet.data_table.columns.created_at', 'Waktu Mutasi'),
        cell: ({ row }) => (
    ...
    },
    {
        ...
        header: () => i18next.t('page.capital_wallet.data_table.columns.tx_type', 'Jenis Transaksi'),
        cell: ({ row }) => {
            const txType = row.original.transaction_type;
            let label: string = txType;
            if (txType === 'capital_injection') {
                label = i18next.t('page.capital_wallet.data_table.filters.tx_capital_injection', 'Suntik Modal');
                ...
                label = i18next.t('page.capital_wallet.data_table.filters.tx_reinvestment', 'Reinvestasi');
            }
    ...
    {
        accessorKey: 'type',
        header: () => i18next.t('page.capital_wallet.data_table.columns.direction', 'Arah Aliran'),
        cell: ({ row }) => {
            ...
            if (type === 'in') {
                return (
                    <Badge className="bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400 border-none font-normal">
                        {i18next.t('page.capital_wallet.data_table.filters.direction_in', 'Uang Masuk')}
                    </Badge>
                );
            }
            return (
                <Badge className="bg-rose-100 text-rose-800 dark:bg-rose-950/40 dark:text-rose-400 border-none font-normal">
                    {i18next.t('page.capital_wallet.data_table.filters.direction_out', 'Uang Keluar')}
                </Badge>
            );
        },
    },
    {
        accessorKey: 'amount',
        header: () => i18next.t('page.capital_wallet.data_table.columns.amount', 'Jumlah'),
        cell: ({ row }) => {
    ...
    },
    {
        ...
        header: () => i18next.t('page.capital_wallet.data_table.columns.balance_before', 'Saldo Awal'),
        cell: ({ row }) => formatRupiah(row.original.balance_before),
    },
    {
        accessorKey: 'balance_after',
        header: () => i18next.t('page.capital_wallet.data_table.columns.balance_after', 'Saldo Akhir'),
        cell: ({ row }) => <span className="font-medium text-foreground">{formatRupiah(row.original.balance_after)}</span>,
    },
    {
        accessorKey: 'notes',
        header: () => i18next.t('page.capital_wallet.data_table.columns.notes', 'Catatan'),
        cell: ({ row }) => <span className="text-muted-foreground line-clamp-1">{row.original.notes || '-'}</span>,
    },
    {
        accessorKey: 'reference',
        header: () => i18next.t('page.capital_wallet.data_table.columns.reference', 'Rujukan'),
        cell: ({ row }) => {
