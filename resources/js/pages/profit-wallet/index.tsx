...
import {
    index as apiGetProfitWallet,
    exportData as apiExportProfitWallet,
    ...
    const fetchProfitWalletData = useCallback(async () => {
        try {
            ...
            const res = await axiosInstance.get<
                ResponseApi<{
                    summary: ProfitWalletSummary;
                    transactions: PaginationResponse<ProfitWalletTransaction>;
                }>
            >(apiGetProfitWallet().url, { params });
            if (res.data.success) {
    ...
    return (
        <>
            ...
            <div className="mb-16 flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
                <HeaderContent>
                    {t('page.profit_wallet.page_name', 'Dompet Profit')}
                </HeaderContent>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    >
                        <CardHeader className="py-4">
                            <CardDescription>
                                {t(
                                    'page.profit_wallet.cards.balance',
                                    'Saldo Berjalan',
                                )}
                            </CardDescription>
                            <CardTitle
                        ...
                        <div className="grid grid-cols-2 gap-2 p-4 pt-0">
                            <Can
                                permission={
                                    PERMISSIONENUMS.PROFIT_WALLET.DISBURSE
                                }
                            >
                                <DisburseDialog
                                    onSuccess={fetchProfitWalletData}
                    ...
                    <Card className="border-l-4 border-l-sky-500 bg-card shadow-xs">
                        <CardHeader className="py-4">
                            <CardDescription>
                                {t(
                                    'page.profit_wallet.cards.inflow',
                                    'Total Uang Masuk',
                                )}
                            </CardDescription>
                    ...
                    <Card className="border-l-4 border-l-rose-500 bg-card shadow-xs">
                        <CardHeader className="py-4">
                            <CardDescription>
                                {t(
                                    'page.profit_wallet.cards.outflow',
                                    'Total Uang Keluar',
                                )}
                            </CardDescription>
