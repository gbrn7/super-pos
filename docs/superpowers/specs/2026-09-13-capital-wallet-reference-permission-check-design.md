# Design Specification: Capital Wallet Reference Column Permission Check

## Overview
Pada modul dompet modal (`resources/js/pages/capital-wallet`), pengguna dapat melihat riwayat transaksi modal. Pada kolom "Rujukan" (`reference`), pengguna dapat mengklik nomor invoice transaksi untuk melihat detail transaksi atau struk. Dokumen ini merinci penyesuaian agar sistem memeriksa apakah pengguna memiliki permission `read-transaction` (`PERMISSIONENUMS.TRANSACTION.READ`). Jika tidak memiliki permission tersebut, kolom rujukan tidak akan ditampilkan di tabel mutasi dompet modal.

## Objectives
1. Memeriksa permission `read-transaction` (`PERMISSIONENUMS.TRANSACTION.READ`) pada halaman Capital Wallet.
...
### 1. `resources/js/pages/capital-wallet/index.tsx`
- **Pengecekan Izin**:
...
### 2. `resources/js/pages/capital-wallet/columns.tsx`
...
2. **Automated Tests**:
   Menjalankan `php artisan test tests/Feature/CapitalWallet/ --compact` untuk memastikan fungsionalitas modal wallet tetap berjalan normal.
